    ...
    <div className="mt-3 rounded-md border border-border bg-inset/80">
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-text">Vitis Doctor</span>
          <span className="text-[10px] text-faint">lokal tanı, dışarı aktarım yok</span>
        </div>
      ...
      <div className="grid gap-3 p-3 lg:grid-cols-[minmax(0,1.2fr)_minmax(220px,0.8fr)]">
        <div className="space-y-2">
          {checks.map((check) => (
            <div key={check.id} className="rounded border border-border bg-bg/50 px-3 py-2">
              <div className="mb-1 flex items-center justify-between gap-2">
                <span className="text-[11px] font-semibold text-text">{check.label}</span>
          ...
          {selfHeal?.attempted ? (
            <div className="rounded border border-accent/30 bg-accent/10 px-3 py-2">
              <div className="mb-1 flex items-center justify-between gap-2">
                <span className="text-[11px] font-semibold text-accent">Self-heal sonucu</span>
                <Badge tone={selfHeal.successful ? "ok" : "warn"}>{selfHeal.successful ? "başarılı" : "başarısız"}</Badge>
              </div>
              <p className="text-[11px] leading-relaxed text-muted">{selfHeal.message}</p>
              {selfHeal.patched_make_libs?.length ? (
                <div className="mt-2 space-y-1">
                  {selfHeal.patched_make_libs.slice(0, 6).map((path) => (
        ...
        <div className="space-y-2">
          ...
          {xsa ? (
            <div className="rounded border border-border bg-bg/50 px-3 py-2 text-[11px]">
              ...
              <div className="grid grid-cols-2 gap-1 text-muted">
                <span>HWH</span><span className="font-mono text-text">{xsa.hwh_count ?? 0}</span>
                <span>make.libs</span><span className="font-mono text-text">{xsa.total}</span>
                <span>riskli</span><span className="font-mono text-text">{xsa.risky}</span>
                <span>source'suz</span><span className="font-mono text-text">{xsa.sourceless}</span>
              </div>
            </div>
          ) : null}
          {workspace ? (
            <div className="rounded border border-border bg-bg/50 px-3 py-2 text-[11px]">
              <div className="mb-1 font-semibold text-text">Workspace BSP</div>
              <div className="grid grid-cols-2 gap-1 text-muted">
                <span>make.libs</span><span className="font-mono text-text">{workspace.total}</span>
                <span>riskli</span><span className="font-mono text-text">{workspace.risky}</span>
                <span>custom match</span><span className="font-mono text-text">{workspace.custom_match}</span>
                <span>source'suz</span><span className="font-mono text-text">{workspace.sourceless}</span>
              </div>
            </div>
          ) : null}
          {doctor?.custom_ip_candidates?.length ? (
            <div className="rounded border border-border bg-bg/50 px-3 py-2 text-[11px]">
              <div className="mb-1 font-semibold text-text">Custom IP adayları</div>
              <div className="space-y-1">
                {doctor.custom_ip_candidates.slice(0, 8).map((item) => (
                  <div key={item.instance} className="min-w-0">
                    <code className="font-mono text-text">{item.instance}</code>
                    {item.ip_name ? <span className="text-faint"> / {item.ip_name}</span> : null}
                  </div>
                ))}
              </div>
            </div>
          ...
          {doctor?.hints?.length ? (
            <div className="rounded border border-border bg-bg/50 px-3 py-2">
              <div className="mb-1 text-[11px] font-semibold text-text">Yerel öneriler</div>
              <ul className="space-y-1 text-[11px] leading-relaxed text-muted">
                {doctor.hints.map((hint) => <li key={hint}>{hint}</li>)}
              </ul>
            </div>
          ) : null}
        </div>
        ...
        <div className="mt-3 rounded-md border border-danger/30 bg-danger/10 p-3 text-[11px] leading-relaxed text-muted">
          ...
          {result.custom_ip_xsa_make_libs_patched?.length || result.custom_ip_make_libs_patched?.length ? (
            <div className="mt-3">
              ...
              <pre className="mt-1 max-h-32 overflow-auto whitespace-pre-wrap rounded border border-border bg-bg p-2 font-mono text-[11px] text-text">
                {[
                  ...(result.custom_ip_xsa_make_libs_patched ?? []).map((path) => `XSA: ${path}`),
                  ...(result.custom_ip_make_libs_patched ?? []).map((path) => `BSP: ${path}`),
                ].join("\n")}
              </pre>
            </div>
          ) : null}
          ...
          {result.shell_app_name ? (
            <div className="md:col-span-2">
              ...
              <div className="text-[10px] text-faint">
                Aynı platformda ikinci uygulama: drivers/ + cit/ + shell/ + src kökünde main.c. Board&apos;da çalıştır her zaman ajanı yükler.
              </div>
            </div>
          ) : null}
