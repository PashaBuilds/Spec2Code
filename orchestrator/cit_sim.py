...
def sim_header(plan: _ChipPlan) -> str:
    ...
    e.ln(f" *   {module}SimKur(&S_sSim, 0x48U); -- spec adresi (i2c_cihazlar tablosuyla ayni)")
    ...
    e.ln(" * @param spSim Simulator.")
    e.ln(" * @param ucAdres 7-bit adres (spec attach.i2c_address).")
    e.ln(" */")
...
def sim_source(plan: _ChipPlan) -> str:
    ...
    e.ln("};")
    e.blank()
    e.ln("/* Register basina yazilabilirlik (rw/wo = TRUE). */")
    e.ln(f"static const unsigned char S_ucArr{pas}SimYazilabilir[{mod}_SIM_REG_SAYISI] = " + "{")
    e.ln("    " + ", ".join("TRUE" if (i in by_off and by_off[i][2]) else "FALSE" for i in range(reg_count)))
    e.ln("};")
    e.blank()
    ...
    e.ln("    if (uiBoy == 0U)")
    e.ln("    {")
    e.ln("        return XST_SUCCESS; /* yalniz adres probe'u: ACK */")
    e.ln("    }")
    ...
    e.ln(f"            if (S_ucArr{pas}SimYazilabilir[ucReg] == TRUE)")
    e.ln("            {")
...
def spi_sim_source(plan: _ChipPlan) -> str:
    ...
    if rows:
        for off, w, reset, name in rows:
            e.ln(f"    {{{_hex(off, 16)}, {_hex(reset & 0xFFFF, 16)}, {'TRUE' if w else 'FALSE'}}}, /* {name} */")
    else:
        e.ln("    {0x0000U, 0x0000U, TRUE},")
    e.ln("};")
    e.blank()
    ...
    e.ln("        }")
    e.ln("    }")
    e.ln("    return TRUE; /* descriptor'da tanimsiz adres: TICS dizisi yazabilsin diye yazilabilir */")
    e.ln("}")
    e.blank()
    if beh:
        beh["behavior"](e, mod, pas, module)
    ...
    e.ln("    {")
    e.ln("        return XST_FAILURE; /* yalniz register cercevesi modellenir */")
    e.ln("    }")
    e.ln("    for (uiIndex = 0U; uiIndex < uiBoy; uiIndex++)")
    e.ln("    {")
    ...
    e.ln("    {")
    e.ln("        spSim->uiYazmaSayac++;")
    e.ln(f"        if ((uiAdres < {mod}_SIM_REG_SAYISI) && ({module}SimYazilabilir(uiAdres) == TRUE))")
    e.ln("        {")
