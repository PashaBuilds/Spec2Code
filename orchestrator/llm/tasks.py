...
def system_prompt(ruleset: dict) -> str:
    ...
    return (
        ...
        f"- Function names MUST match: {naming.get('function_regex')} and use camelCase, "
        "for example int tca9548aChannelSelect(XIicPs* spIic, unsigned char ucChannel).\n"
        f"- Identifiers use {naming.get('identifier_case', 'camelCase')} with Hungarian prefixes: {prefixes}.\n"
        "- Use primitive C types such as unsigned char, unsigned short, unsigned int, int, "
        "short, long, and unsigned long long; do not use uint8_t/uint16_t/uint32_t style fixed-width typedefs.\n"
        "- Pointer '*' attaches to the previous type for every pointer declaration, "
        "for example XIicPs* spIic and unsigned char* ucpValue.\n"
