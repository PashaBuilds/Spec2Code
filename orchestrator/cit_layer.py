...
def sistem_source(plans: list[_ChipPlan], spec: Optional[dict] = None) -> str:
    ...
    e.ln("    {")
    e.ln('        dbg_printf(DEBUG_LEVEL_INFO, "|   %-16s %10d %-7s %-19s %-4s |", cpAd, iDeger, cpBirim, "(kapali)", "-");')
    e.ln("    }")
    e.ln("    else if (spLimit->uiLimitVar == 1U)")
    e.ln("    {")
    e.ln('        dbg_printf(DEBUG_LEVEL_INFO, "|   %-16s %10d %-7s [%8d..%8d] %-4s |", cpAd, iDeger, cpBirim, spLimit->iMin, spLimit->iMax,')
    e.ln('                   (uiOk == TRUE) ? "OK" : "NOK");')
    e.ln("    }")
    e.ln("    else")
    e.ln("    {")
    e.ln('        dbg_printf(DEBUG_LEVEL_INFO, "|   %-16s %10d %-7s %-19s %-4s |", cpAd, iDeger, cpBirim, "limitsiz", (uiOk == TRUE) ? "OK" : "NOK");')
    e.ln("    }")
    e.ln("}")
    e.blank()
    e.ln("static void sistemCitCihazBaslik(const char* cpCihaz, int iSonuc)")
    e.ln("{")
    e.ln('    dbg_printf(DEBUG_LEVEL_INFO, "| %-60s %-7s |", cpCihaz, sistemCitSonucAdi(iSonuc));')
    e.ln("}")
    for c in controllers:
        htype, _ = cmodel._handle_for(c)
    ...
    e.ln('    dbg_printf(DEBUG_LEVEL_INFO, "| CIT kosusu #%-56u |", uiSayac);')
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIZGI);")
    for plan in plans:
    ...
    e.ln('    dbg_printf(DEBUG_LEVEL_INFO, "| SONUC: %-61s |", sistemCitSonucAdi(iEnKotu));')
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIZGI);")
    e.ln("    return iEnKotu;")
    e.ln("}")
    return e.text()
