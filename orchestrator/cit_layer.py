...
hangi olcumlerin okunacagi, limit (min/max), etkin/kritik, OK/NOK bitleri, hata/NOK
...
typedef struct
{
    int iMin;                    /* alt limit (uiLimitVar == 1 ise gecerli)          */
    int iMax;                    /* ust limit                                         */
    unsigned int uiLimitVar : 1; /* 0 = limitsiz (okundu ise OK)                     */
    unsigned int uiEtkin : 1;    /* 0 = olcum degerlendirilmez (OK sayilir)           */
    unsigned int uiKritik : 1;   /* 1 = critical, 0 = warning (ust katman bilgisi)    */
} SCitLimit;

/**
 * @brief Degeri limite gore degerlendirir.
 * @return 1 = OK (etkin degil, limit yok ya da aralikta), 0 = NOK.
 */
...
def _limit_initializer(olcum: dict) -> str:
    ...
    kritik = 1 if str(olcum.get("severity", "warning")) == "critical" else 0
    return f"{{{int(mn) if has else 0}, {int(mx) if has else 0}, {1 if has else 0}U, {etkin}U, {kritik}U}}"
...
def chip_header(plan: _ChipPlan) -> str:
    ...
    e.ln(" * limit/etkin/kritik politikasini uygular ve OK/NOK bitlerini doldurur.")
...
def readme_section(plans: list[_ChipPlan], skipped: list[tuple[str, str]]) -> str:
    lines = [
        ...
        "okunur, limit/etkin/kritik politikasi, OK/NOK bitleri. Ikisini birlikte kendi yazilimina tasi.",
        "",
        "| Dosya | Icerik |",
        "|---|---|",
        "| `cit/cit_ortak.h/.c` | `SCitLimit`, `citLimitDegerlendir()`, `CIT_OK/NOK/HATA` |",
