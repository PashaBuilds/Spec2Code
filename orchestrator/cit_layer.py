...
def sistem_source(plans: list[_ChipPlan], spec: Optional[dict] = None) -> str:
    ...
    e.ln(" * OK yesil, NOK kirmizi, HATA sari, kapali/limitsiz-bilgi gri. Terminal (XShell/PuTTY)")
    ...
    e.ln('#define SISTEM_CIT_RENK_GRI "\\033[90m"')
    ...
    e.blank()
    e.ln("    if (spLimit->uiEtkin == 0U)")
    e.ln("    {")
    e.ln('        (void)snprintf(cArrLimit, sizeof(cArrLimit), "%s", "(kapali)");')
    e.ln("        cpRenk = SISTEM_CIT_RENK_GRI;")
    e.ln('        cpSonuc = "-";')
    e.ln("    }")
    e.ln("    else")
    e.ln("    {")
