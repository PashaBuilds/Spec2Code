...
typedef struct
{
    ...
    unsigned int uiLimitVar : 1; /* 1 = [iMin, iMax] uygulanir; 0 = limitsiz (okundu ise OK) */
} SCitLimit;

/**
 * @brief Degeri limite gore degerlendirir: iMin <= iDeger <= iMax ise OK.
 * @return TRUE = OK (limit yok ya da aralikta), FALSE = NOK (aralik disi).
 */
...
unsigned int citLimitDegerlendir(const SCitLimit* spLimit, int iDeger)
{
    if ((spLimit == (const SCitLimit*)0) || (spLimit->uiLimitVar == 0U))
    {
        return TRUE;
    }
...
def _limit_initializer(olcum: dict) -> str:
    ...
    """
    has = _limit_present(olcum)
    mn, mx = olcum.get("min"), olcum.get("max")
    return (f"{{.iMin = {int(mn) if has else 0}, .iMax = {int(mx) if has else 0}, "
            f".uiLimitVar = {1 if has else 0}U}}")


def _limit_present(olcum: dict) -> bool:
    """Limit var mi: min VE max sayisal VE olcum etkin. Tek alan (uiLimitVar): 'etkin degil' ==
    'limitsiz' (SAHA istegi 2026-09-08: ayri uiEtkin alani gereksizdi)."""
    mn, mx = olcum.get("min"), olcum.get("max")
    return bool(olcum.get("enabled", True)) and isinstance(mn, (int, float)) and isinstance(mx, (int, float))


def _limit_note(olcum: dict, unit: str) -> str:
    if _limit_present(olcum):
        return f"[{int(olcum['min'])} .. {int(olcum['max'])}]" + (f" {unit}" if unit else "")
    return "limitsiz"
    ...
    for m in plan.measures:
        for ch in m.channels:
            ...
            if "name" in olcum:
                ...
                # Modulde olup bu cihazda istenmeyen olcum: alan yapida var, limitsiz.
                note = f"{str(plan.device.get('id', '')).upper()}_{ch.label.upper()}: bu cihazda olcum yok (limitsiz)"
...
def chip_header(plan: _ChipPlan) -> str:
    ...
    e.ln(" * limit politikasini uygular ve OK/NOK bitlerini doldurur (aralik kapali: min <= deger <= max).")
...
def sistem_header(plans: list[_ChipPlan], skipped: list[tuple[str, str]], spec: Optional[dict] = None) -> str:
    ...
    e.ln(" * Kendi main'inde kopyalayip degerleri degistir; SCitLimit = {iMin, iMax, uiLimitVar};")
    e.ln(" * uiLimitVar = 0 -> limitsiz (okundu ise OK). */")
...
def sistem_source(plans: list[_ChipPlan], spec: Optional[dict] = None) -> str:
    ...
    e.blank()
    e.ln("/* --- CIT raporu (DEBUG_LEVEL_INFO): her kosuda cerceveli tablo ----------------------")
    e.ln(" * Her satir 72 sutun; her entegre kendi kutusunda. Satirlar ANSI renkle basilir:")
    e.ln(" * OK yesil, NOK kirmizi, HATA sari. Terminal (XShell/PuTTY)")
    e.ln(" * renkleri dogrudan gosterir; host Akis ekrani kodu cozup ayni tonu uygular. */")
    e.ln('#define SISTEM_CIT_GENISLIK 70U /* kutu ic genisligi (kenar cizgileri haric) */')
    e.ln('#define SISTEM_CIT_CIZGI "+----------------------------------------------------------------------+"')
    e.ln('#define SISTEM_CIT_CIFT_CIZGI "+======================================================================+"')
    e.ln('#define SISTEM_CIT_RENK_OK "\\033[32m"')
    e.ln('#define SISTEM_CIT_RENK_NOK "\\033[31m"')
    e.ln('#define SISTEM_CIT_RENK_HATA "\\033[33m"')
    e.ln('#define SISTEM_CIT_RENK_SIFIRLA "\\033[0m"')
    e.blank()
    e.ln("static const char* sistemCitSonucAdi(int iSonuc)")
    e.ln("{")
    e.ln(f"    if (iSonuc == {STATUS_OK})")
    e.ln("    {")
    e.ln('        return "OK";')
    e.ln("    }")
    e.ln(f"    if (iSonuc == {STATUS_NOK})")
    e.ln("    {")
    e.ln('        return "NOK";')
    e.ln("    }")
    e.ln('    return "HATA";')
    e.ln("}")
    e.blank()
    e.ln("static const char* sistemCitSonucRengi(int iSonuc)")
    e.ln("{")
    e.ln(f"    if (iSonuc == {STATUS_OK})")
    e.ln("    {")
    e.ln("        return SISTEM_CIT_RENK_OK;")
    e.ln("    }")
    e.ln(f"    if (iSonuc == {STATUS_NOK})")
    e.ln("    {")
    e.ln("        return SISTEM_CIT_RENK_NOK;")
    e.ln("    }")
    e.ln("    return SISTEM_CIT_RENK_HATA;")
    e.ln("}")
    e.blank()
    e.ln("/* Olcum satiri: ad(18) deger(10) birim(7) limit(24) sonuc(4) = 70 sutun. */")
    e.ln("static void sistemCitSatir(const char* cpAd, int iDeger, const char* cpBirim, const SCitLimit* spLimit, unsigned int uiOk)")
    e.ln("{")
    e.ln("    char cArrLimit[32];")
    e.ln("    const char* cpRenk;")
    e.ln("    const char* cpSonuc;")
    e.blank()
    e.ln("    if (spLimit->uiLimitVar == 1U)")
    e.ln("    {")
    e.ln('        (void)snprintf(cArrLimit, sizeof(cArrLimit), "[%d .. %d]", spLimit->iMin, spLimit->iMax);')
    e.ln("    }")
    e.ln("    else")
    e.ln("    {")
    e.ln('        (void)snprintf(cArrLimit, sizeof(cArrLimit), "%s", "limitsiz");')
    e.ln("    }")
    e.ln("    cpRenk = (uiOk == TRUE) ? SISTEM_CIT_RENK_OK : SISTEM_CIT_RENK_NOK;")
    e.ln('    cpSonuc = (uiOk == TRUE) ? "OK" : "NOK";')
    e.ln('    dbg_printf(DEBUG_LEVEL_INFO, "%s|  %-18.18s %10d %-7.7s %-24.24s %4s |%s", cpRenk, cpAd, iDeger, cpBirim, cArrLimit, cpSonuc,')
    e.ln("               SISTEM_CIT_RENK_SIFIRLA);")
    e.ln("}")
    e.blank()
    e.ln("/* Entegre kutusu basligi: ust cizgi + ad(61) sonuc(6) satiri + ayirici. Kutunun alti bir")
    e.ln(" * sonraki entegrenin ust cizgisi ya da SONUC'un cift cizgisidir (cift cizgi basilmaz). */")
    e.ln("static void sistemCitCihazBaslik(const char* cpCihaz, int iSonuc)")
    e.ln("{")
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIZGI);")
    e.ln('    dbg_printf(DEBUG_LEVEL_INFO, "%s| %-61.61s %6s |%s", sistemCitSonucRengi(iSonuc), cpCihaz, sistemCitSonucAdi(iSonuc),')
    e.ln("               SISTEM_CIT_RENK_SIFIRLA);")
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIZGI);")
    e.ln("}")
    for c in controllers:
    ...
    e.ln(f'    dbg_printf(DEBUG_LEVEL_INFO, "| CIT kosusu #%-5u %-50.50s |", uiSayac, "{(spec or {}).get("project", {}).get("name", "")}");')
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIFT_CIZGI);")
    for plan in plans:
        ...
        e.ln("    }")
        e.ln(f'    sistemCitCihazBaslik("{plan.device["id"]} ({plan.part})", iStatus);')
        for m in plan.measures:
            for ch in m.channels:
    ...
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIFT_CIZGI);")
    e.ln('    dbg_printf(DEBUG_LEVEL_INFO, "%s| SONUC: %-61.61s |%s", sistemCitSonucRengi(iEnKotu), sistemCitSonucAdi(iEnKotu),')
    e.ln("               SISTEM_CIT_RENK_SIFIRLA);")
    e.ln("    dbg_printf(DEBUG_LEVEL_INFO, SISTEM_CIT_CIFT_CIZGI);")
    e.ln("    return iEnKotu;")
    e.ln("}")
...
def readme_section(plans: list[_ChipPlan], skipped: list[tuple[str, str]]) -> str:
    lines = [
        ...
        "okunur, limit politikasi (kapali aralik, min == max gecerli; limitsiz = okundu ise OK), OK/NOK bitleri. Ikisini birlikte kendi yazilimina tasi.",
        "",
        "| Dosya | Icerik |",
        "|---|---|",
        "| `cit/cit_ortak.h/.c` | `SCitLimit`, `citLimitDegerlendir()`, `CIT_OK/NOK/HATA` |",
