...
def _mesaj_header(spec: dict | None = None,
    ...
    return (
        ...
        "#define SPEC2CODE_MESAJ_CIT_LIMIT_BOY 12U /* CIT_LIMIT_SET: SCitLimit tel boyu (iMin, iMax, uiLimitVar) */\n"
...
_MESAJ_CIT_BRANCH_ENABLED = (
    ...
    "        {\n"
    "            /* Canli limitler: std govde (uiUzunluk = N) + N x SCitLimit (12 B); karar KARTTA. */\n"
...
def _testbench_manifest_devices(spec: dict, get_descriptor: Callable[[str], dict]) -> list[dict]:
    ...
    for device in spec.get("devices", []):
        ...
        # "simulated" YALNIZ isaretliyken yazilir: isaretsiz projelerde manifest
        # bugunkuyle bayt-bayt ayni kalir.
        if device.get("simulate"):
            board_fields = {**board_fields, "simulated": True}
        devices.append({
            "id": device.get("id", ""),
...
def _testbench_gpio_lines(handle_types: set[str]) -> list[str]:
    ...
    return [
        ...
        "            /* TRI geri okumasi GUVENILMEZ: \"All Outputs\" kanalinda TRI yoktur ve",
        "             * sabit 0xFFFFFFFF okunur (SAHA 2026-09-13 Nexys A7 LED kanali) - o",
        "             * kanal tam da yazilabilen kanaldir. \"All Inputs\" kanalinda ise DATA",
        "             * yazmasi yutulur ve geri okuma pin girislerini verir. Bu yuzden",
        "             * dogrulama DATA geri okumasiyla yapilir. */",
        ...
        "            if (uiCurrent != (spRequest->uiValue & uiMask))",
        "            {",
        "                spResponse->iStatus = XST_FAILURE;",
        "                spResponse->uiValue = uiCurrent;",
        "                spec2codeTestbenchMessageSet(spResponse, \"gpio_write: geri okuma farkli (kanal All Inputs / salt-okunur?)\");",
        "                return XST_FAILURE;",
        "            }",
        "        }",
...
def _testbench_ops_source(spec: dict, get_descriptor: Callable[[str], dict]) -> str:
    ...
    lines.extend([
        ...
        "        return XST_SUCCESS;",
        "    }",
        # Built-in: ardisik 32-bit sozcuk blogu (yakalama BRAM'i, Spec2Code Yakalama ekrani).
        # length = sozcuk sayisi (yanit veri tamponuna sigacak kadar kirpilir), value = okunan sozcuk.
        "    if (spec2codeTestbenchStringEqual(spRequest->cArrOperation, \"mem_block\") == TRUE)",
        "    {",
        "        unsigned int uiWordCount = spRequest->uiLength;",
        "        unsigned int uiWordIndex;",
        "        unsigned int uiByteIndex;",
        "        unsigned int uiWord;",
        "        if (uiWordCount > (SPEC2CODE_TESTBENCH_DATA_MAX / 4U))",
        "        {",
        "            uiWordCount = SPEC2CODE_TESTBENCH_DATA_MAX / 4U;",
        "        }",
        ...
        "        spResponse->uiOk = 1U;",
        "        spResponse->iStatus = XST_SUCCESS;",
        "        spResponse->uiValue = uiWordCount;",
