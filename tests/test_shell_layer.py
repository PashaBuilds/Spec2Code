...
class ShellLayerGenerationTests(unittest.TestCase):
    ...
    def test_tree_and_no_product_names(self) -> None:
        for rel in ("shell/shell_uart.h", "shell/shell_uart.c", "shell/shell.h", "shell/shell.c", "main.c", "main.h",
                    "shell/shell_user_commands.h", "shell/shell_user_commands.c", "shell/shell_cmd_args.h", "shell/shell_cmd_args.c",
                    "shell/shell_cmd_mem.h", "shell/shell_cmd_mem.c", "shell/shell_cmd_cit.h", "shell/shell_cmd_cit.c",
                    "shell/shell_cmd_i2c.h", "shell/shell_cmd_i2c.c", "shell/shell_cmd_sys.h", "shell/shell_cmd_sys.c",
                    "shell/shell_cmd_mod.h", "shell/shell_cmd_mod.c"):
    ...
    def test_api_and_commands(self) -> None:
        ...
        # Cekirdekte komut yok; tablo shell_user_commands.c'de, isleyiciler islev gruplarina gore
        # shell_cmd_<grup>.c dosyalarinda (kullanici istegi 2026-09-16: tek dosyada toplanmasin).
        ...
        # Komut tablosu: yalniz tablo, isleyici tanimi yok; her grup basligi include edilir.
        table = _read(self.out_dir, "shell/shell_user_commands.c")
        self.assertIn("static const SShellCommand S_sArrUserCommands[] = {", table)
        self.assertNotIn("uiArgc, const char* cpArrArgv[])\n{", table)
        for group in ("cit", "i2c", "mem", "sys", "mod"):
            self.assertIn(f'#include "shell_cmd_{group}.h"', table)
        cit = _read(self.out_dir, "shell/shell_cmd_cit.c")
        i2c = _read(self.out_dir, "shell/shell_cmd_i2c.c")
        mem = _read(self.out_dir, "shell/shell_cmd_mem.c")
        sysc = _read(self.out_dir, "shell/shell_cmd_sys.c")
        mod = _read(self.out_dir, "shell/shell_cmd_mod.c")
        args = _read(self.out_dir, "shell/shell_cmd_args.c")
        # Isleyiciler disa acik (static degil) ve grup basliginda bildirilmis.
        for group, handler in (("cit", "shellUserCit"), ("i2c", "shellUserI2cSearch"), ("i2c", "shellUserI2cRead"),
                               ("i2c", "shellUserI2cWrite"), ("mem", "shellUserMem"), ("mem", "shellUserMemPcieIntr0"),
                               ("sys", "shellUserSdl"), ("sys", "shellUserHelp"), ("mod", "shellUserMod")):
            self.assertIn(f"void {handler}(unsigned int uiArgc, const char* cpArrArgv[]);", _read(self.out_dir, f"shell/shell_cmd_{group}.h"))
            self.assertIn(f"\nvoid {handler}(unsigned int uiArgc, const char* cpArrArgv[])\n", _read(self.out_dir, f"shell/shell_cmd_{group}.c"))
        # Ortak arguman ayristirma tek yerde; mem ve i2c onu kullanir.
        self.assertIn("unsigned int shellUserParseNumber(const char* cpText, unsigned long* ulpValue)", args)
        self.assertNotIn("shellUserParseNumber(const char* cpText", mem)
        self.assertIn('#include "shell_cmd_args.h"', mem)
        self.assertIn('#include "shell_cmd_args.h"', i2c)
        user = cit + i2c + mem + sysc + mod
        for command, handler in (("cit", "shellUserCit"), ("i2c_search", "shellUserI2cSearch"),
            ...
            self.assertIn(f'{{"{command}", {handler}, "', table)
        ...
        self.assertNotIn('"i2c_bus"', table)
        self.assertNotIn("shellUserI2cBus(", i2c)
    ...
    def test_main_wires_banner_and_init_before_loop(self) -> None:
        # main.c/main.h KOK dizinde (kullanici istegi: src/ altinda tek basina), shell/ icinde degil.
        main = _read(self.out_dir, "main.c")
        self.assertTrue((self.out_dir / "main.h").is_file())
        self.assertFalse((self.out_dir / "shell" / "main.c").exists())
        self.assertFalse((self.out_dir / "shell" / "main.h").exists())
...
class ShellLayerPlatformTests(unittest.TestCase):
    def test_zynqmp_uses_ps_uart_and_xiicps_probe(self) -> None:
        ...
        try:
            ...
            source = _read(out_dir, "shell/shell_cmd_i2c.c")
