...
Tasarim: docs/superpowers/specs/2026-09-06-driver-struct-api-design.md
  ...
  * drivers/: S<Mod>Status (bit bit) + <mod>StatusRegistersRead, dizi op'lari icin
    S<Mod>Voltage; kullaniciya giden dosyalarda `spec2code` adi YOK (bus_trace.h).
...
class DriverStructApiTests(unittest.TestCase):
    ...
    def test_user_facing_files_have_no_spec2code_names(self) -> None:
        ...
        self.assertTrue((drivers / "bus_trace.h").is_file())
        trace = _read(self.out_dir, "drivers/bus_trace.h")
        self.assertIn("void busTraceI2c(", trace)
        self.assertIn("BUS_TRACE_NO_WEAK", trace)
        self.assertIn("busTraceI2cError(LTC2991_I2C_ADDR, ucReg, 'p', iStatus);", _read(self.out_dir, "drivers/ltc2991.c"))
...
class CitLayerGenerationTests(unittest.TestCase):
    ...
    def test_ltc2991_cit_limits_flags_and_driver_calls(self) -> None:
        ...
        # spec limiti varsayilana gomulur: {min, max, limitVar, etkin, kritik}
        self.assertIn("{3135, 3465, 1U, 1U, 1U},", header)
...
class SimulatedDeviceAgentTests(unittest.TestCase):
    ...
    def test_simulated_devices_register_simulators_and_no_shims(self) -> None:
        ...
        try:
            ...
            self.assertIn("ltc2991SimKur(&S_sSimU2Ltc2991, LTC2991_I2C_ADDR);", ops)
...
int main(void)
{
    ...
    /* V1 spec limiti 3135..3465 (kritik): 3300 OK; 2000'e cekince NOK */
    ltc2991SimKanalAyarla(&S_sLtc, 0U, 2000);
    iRead = sistemCitRead(&S_sBus, &sLimit, &S_sCit);
    printf("D read=%d v1ok=%u v1=%u nok=%u kritik=%u\n", iRead, S_sCit.sU2Ltc2991.sBayraklar.uiV1Ok,
           S_sCit.sU2Ltc2991.sVoltage.usArrVoltage[0], S_sCit.uiNokSayac, sLimit.sU2Ltc2991.sV1.uiKritik);
...
class CitLayerHostRoundTripTests(unittest.TestCase):
    ...
    def test_system_read_over_virtual_devices(self) -> None:
        ...
        try:
            ...
            # D: V1 limit disi -> NOK (kritik)
            self.assertIn("read=1 v1ok=0 v1=1999 nok=1 kritik=1", lines["D"])
