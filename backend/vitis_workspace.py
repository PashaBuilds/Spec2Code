...
def render_xsct_script(
    *,
    ...
    bsp_config_script = (
        ...
        f"                {_tcl_put('MicroBlaze: lwIP bellek havuzlari LMB icin kucultuluyor (mem_size 32768, pbuf_pool_size 16, tcp_snd_buf 4096 >= 2*MSS)')}"
        "                foreach {spec2code_lwip_param spec2code_lwip_value} {mem_size 32768 memp_n_pbuf 8 pbuf_pool_size 16 tcp_wnd 4096 tcp_snd_buf 4096 memp_n_tcp_pcb 4 memp_n_tcp_seg 64} {\n"
        "                    if {[catch {bsp config $spec2code_lwip_param $spec2code_lwip_value} spec2code_lwip_param_err]} {\n"
        f"                        {_tcl_put('lwIP $spec2code_lwip_param ayarlanamadi (BSP varsayilani kalir): $spec2code_lwip_param_err')}"
        "                    }\n"
        "                }\n"
        "            }\n"
        ...
        "            }\n"
        "            spec2codeDisableCustomIpBspLibsrc\n"
        "            spec2codePatchLwipEmacliteStatus\n"
        "        } else {\n"
        ...
        "    }\n"
        "}\n\n"
        "proc spec2codeFindFiles {dir name result_var} {\n"
        "    upvar 1 $result_var result\n"
        "    foreach f [glob -nocomplain -directory $dir -types f $name] { lappend result $f }\n"
        "    foreach d [glob -nocomplain -directory $dir -types d *] { spec2codeFindFiles $d $name result }\n"
        "}\n\n"
        "proc spec2codePatchLwipPhyAutonegTimeout {} {\n"
        "    # Xilinx lwip213 xemacpsif_physpeed.c (ZynqMP GEM) PHY autonegotiation beklemesi 5 s (SAHA 2026-09-15,\n"
        "    # KV260 DP83867 + Intel I225-V): yazilim resetinden sonra gigabit autoneg 5 s'de bitmiyor, port\n"
        "    # 'Auto negotiation error' verip GEM hizini ayarlamiyor. Bekleme 30 s'ye cikarilir (her PHY yolu).\n"
        "    global workspace_path\n"
        "    set files [list]\n"
        "    spec2codeFindFiles $workspace_path xemacpsif_physpeed.c files\n"
        "    foreach f $files {\n"
        "        if {[string first \"lwip\" $f] < 0} { continue }\n"
        "        set fh [open $f r]\n"
        "        set text [read $fh]\n"
        "        close $fh\n"
        "        set fixed [string map [list \"timeout_counter == 5)\" \"timeout_counter == 30) /* Spec2Code: autoneg 30 s */\"] $text]\n"
        "        if {$fixed ne $text} {\n"
        "            set fh [open $f w]\n"
        "            puts -nonewline $fh $fixed\n"
        "            close $fh\n"
        f"            {_tcl_put('lwIP PHY autonegotiation timeout patched (5 s -> 30 s): $f')}"
        "        }\n"
        "    }\n"
        "}\n\n"
        "proc spec2codePatchLwipEmacliteStatus {} {\n"
        "    # Xilinx lwip213 v1.0/v1.1 xadapter.c hatasi (SAHA 2026-09-11, Vitis 2023.2): emaclite_link_status\n"
        "    # icinde `status` hem u32_t hem u16_t bildirilmis -> EmacLite (MicroBlaze) BSP derlemesi\n"
        "    # 'conflicting types for status' ile duser. u32_t satirindan `status` cikarilir (u16_t kalir;\n"
        "    # XEmacLite_PhyRead u16_t* bekler). Dosya yoksa (GEM/ZynqMP) hicbir sey yapilmaz.\n"
        "    # Ikinci hata (SAHA 2026-09-11, Nexys A7 LAN8720A): xemacliteif.c ADVERTISE_100_AND_10 / _100 / _10\n"
        "    # IEEE 802.3 secici alanini (bit0 = 0x0001) tasimiyor -> PHY reg4 = 0x01E0 -> autoneg hic\n"
        "    # tamamlanmaz, link duser, port sonsuz dongude bekler. JTAG ile reg4 = 0x01E1 yazilinca link\n"
        "    # aninda geldi. Secici eklenir.\n"
        "    global workspace_path\n"
        "    set files [list]\n"
        "    spec2codeFindFiles $workspace_path xadapter.c files\n"
        "    spec2codeFindFiles $workspace_path xemacliteif.c files\n"
        "    foreach f $files {\n"
        "        if {[string first \"lwip213\" $f] < 0} { continue }\n"
        "        set fh [open $f r]\n"
        "        set text [read $fh]\n"
        "        close $fh\n"
        "        set fixed [string map [list \\\n"
        "            \"u32_t phy_link_status, status, phy_autoneg_status;\" \"u32_t phy_link_status, phy_autoneg_status; /* Spec2Code: duplicate status fixed */\" \\\n"
        "            \"#define ADVERTISE_100_AND_10\\t(ADVERTISE_10FULL | ADVERTISE_100FULL |\" \"#define ADVERTISE_100_AND_10\\t(0x0001 /* Spec2Code: IEEE 802.3 selector */ | ADVERTISE_10FULL | ADVERTISE_100FULL |\" \\\n"
        "            \"#define ADVERTISE_100\\t\\t(ADVERTISE_100FULL | ADVERTISE_100HALF)\" \"#define ADVERTISE_100\\t\\t(0x0001 | ADVERTISE_100FULL | ADVERTISE_100HALF)\" \\\n"
        "            \"#define ADVERTISE_10\\t\\t(ADVERTISE_10FULL | ADVERTISE_10HALF)\" \"#define ADVERTISE_10\\t\\t(0x0001 | ADVERTISE_10FULL | ADVERTISE_10HALF)\"] $text]\n"
        "        if {$fixed ne $text} {\n"
        "            set fh [open $f w]\n"
        "            puts -nonewline $fh $fixed\n"
        "            close $fh\n"
        f"            {_tcl_put('lwIP emaclite source patched (duplicate status / IEEE selector): $f')}"
        "        }\n"
        "    }\n"
        "}\n\n"
        "proc spec2codePatchLinkerStack {ld_path} {\n"
        "    # MicroBlaze/Zynq app'lerinde Vitis'in varsayilan yigini 1 KB (0x400): test bench\n"
        "    # ajaninin CIT + surucu + dbg_printf yolu bunu asar (SAHA: Nexys A7, PC ASCII metne\n"
        "    # dondu). Yigin 16 KB, heap 8 KB yapilir; zaten buyukse dokunulmaz.\n"
        "    if {![file exists $ld_path]} {\n"
        f"        {_tcl_put('linker script not found, stack left as is: $ld_path')}"
        "        return\n"
        "    }\n"
        "    set f [open $ld_path r]\n"
        "    set txt [read $f]\n"
        "    close $f\n"
        "    set changed 0\n"
        "    if {[regexp -line {^_STACK_SIZE = DEFINED\\(_STACK_SIZE\\) \\? _STACK_SIZE : 0x([0-9A-Fa-f]+);} $txt -> cur]} {\n"
        "        if {[expr 0x$cur] < 0x4000} {\n"
        "            regsub -line {^_STACK_SIZE = DEFINED\\(_STACK_SIZE\\) \\? _STACK_SIZE : 0x[0-9A-Fa-f]+;} $txt {_STACK_SIZE = DEFINED(_STACK_SIZE) ? _STACK_SIZE : 0x4000;} txt\n"
        "            set changed 1\n"
        "        }\n"
        "    }\n"
        "    if {[regexp -line {^_HEAP_SIZE = DEFINED\\(_HEAP_SIZE\\) \\? _HEAP_SIZE : 0x([0-9A-Fa-f]+);} $txt -> cur]} {\n"
        "        if {[expr 0x$cur] < 0x2000} {\n"
        "            regsub -line {^_HEAP_SIZE = DEFINED\\(_HEAP_SIZE\\) \\? _HEAP_SIZE : 0x[0-9A-Fa-f]+;} $txt {_HEAP_SIZE = DEFINED(_HEAP_SIZE) ? _HEAP_SIZE : 0x2000;} txt\n"
        "            set changed 1\n"
        "        }\n"
        "    }\n"
        "    if {$changed} {\n"
        "        set f [open $ld_path w]\n"
        "        puts -nonewline $f $txt\n"
        "        close $f\n"
        f"        {_tcl_put('linker script stack/heap raised (stack 0x4000, heap 0x2000): $ld_path')}"
        "    } else {\n"
        f"        {_tcl_put('linker script stack/heap already sufficient: $ld_path')}"
        "    }\n"
        "}\n\n"
        ...
        "    spec2codeDisableCustomIpBspLibsrc\n"
        "    spec2codePatchLwipEmacliteStatus\n"
        "    spec2codePatchLwipPhyAutonegTimeout\n"
    ...
    return (
        ...
        "    set spec2code_sim_flag {-include spec2code_sim_xilinx.h}\n"
        "    set spec2code_misc {}\n"
        "    catch {set spec2code_misc [app config -name $app_name -get compiler-misc]}\n"
        "    if {[string first $spec2code_sim_flag $spec2code_misc] < 0} {\n"
        "        if {[catch {app config -name $app_name -add compiler-misc $spec2code_sim_flag} spec2code_sim_err]} {\n"
        f"            {_tcl_put('WARNING: sim -include flag not added: $spec2code_sim_err')}"
        "        } else {\n"
        f"            {_tcl_put('sim interposer flag added: $spec2code_sim_flag')}"
        "        }\n"
        "    }\n"
        "}\n\n"
